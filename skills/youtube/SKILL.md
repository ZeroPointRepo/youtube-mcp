...
- Returns basic metadata (title, author, thumbnail) plus available transcript languages. Call this before `get_youtube_transcript` to pick a language.
...
- Prefer it over `list_channel_videos` when recent uploads are enough, and use it freely to check what a creator posted.
...
- Successful calls cost 1 credit unless a tool states otherwise below. Failed and rate-limited calls do not consume credits.
